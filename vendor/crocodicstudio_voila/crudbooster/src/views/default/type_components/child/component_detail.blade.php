            ...
            <div class="panel-body">
                <table id='table-{{ $name }}' class='table table-striped table-bordered'>
                    ...
                    <tbody>
                        ...
                        <?php
                    $columns_tbody = [];
                    if(CrudBooster::getCurrentModule()->path  == 'deals'){
                        $return_url = Request::get("return_url");
                        $parameters = optional(parse_url($return_url))['query'];
                        if($parameters){
                            $parameters = explode('&', $parameters);    
                            $parameters = explode('=', $parameters[1]);    
                            $study_engineer = $parameters[1];
                            if($study_engineer)
                                $study_engineer_id = DB::table('cms_users')->where("num", $study_engineer)->first()->id;
                        }
                    }
                    
                    if(CrudBooster::getCurrentModule()->path  == 'deals' && CrudBooster::me()->id_cms_privileges == 2){
                        $data_child = DB::table($form['table'])->where($form['foreign_key'], $id)->where("study_engineer_id",CrudBooster::me()->id);
                    } else if(CrudBooster::getCurrentModule()->path  == 'deals' && CrudBooster::me()->id_cms_privileges == 1 && $study_engineer_id) {
                        $data_child = DB::table($form['table'])->where($form['foreign_key'], $id)->where("study_engineer_id",$study_engineer_id);
                    } else {
